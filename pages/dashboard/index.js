...
function getTendency(entries, croissant, highImpactCount, topReason) {
  if (!entries.length) return { title: "还没有足够记录", text: "现在还不急着判断。先写下一条真实的记录，让混乱有个落脚点。" };
  if (croissant.statusKey === "protect" || (topReason === "成长与转向" && entries.length >= 3)) return { title: "启动过渡计划", text: "你可能已经不只是想逃离，而是在寻找新的生活结构。先别硬跳，也别无限拖延，给自己搭一座小桥。" };
...
Page({
  data: { ranges: RANGES, activeRange: "30", hasRecords: false, croissant: getCroissantReport([]), stats: { total: 0, negative: 0, positive: 0, highImpact30Days: 0, topReason: "还没有足够记录", advice: "本周先保护好自己。第一步不是判断，而是记录。" }, reasonDistribution: [], highImpactEntries: [], tendency: {}, reviewText: "" },
  onShow() { this.refresh(); },
  ...
  refresh() {
    ...
    this.setData({
      hasRecords,
      croissant,
      stats: { total: entries.length, negative: entries.filter((entry) => entry.type === "negative").length, positive: entries.filter((entry) => entry.type === "positive").length, highImpact30Days: highImpactEntries.length, topReason, advice: hasRecords ? PROTECTION_ADVICE[topReason] || croissant.advice : "本周先保护好自己。第一步不是判断，而是记录。" },
      reasonDistribution,
      highImpactEntries,
      tendency,
